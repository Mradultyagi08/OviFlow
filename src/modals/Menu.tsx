  ...
  return (
    <IonItem>
      ...
      <IonSelect
        ...
        onIonChange={(event) => updateAppMode(event.target.value as string)}
      >
        <div slot="label">
          <IonText color={`text-${theme}`}>{t("Mode")}</IonText>
        </div>
        <IonSelectOption value="regular">{t("Regular")}</IonSelectOption>
        <IonSelectOption value="pregnancy">
          {t("Pregnancy Care")}
        </IonSelectOption>
        <IonSelectOption value="postpartum">
          {t("Postpartum Care")}
        </IonSelectOption>
      </IonSelect>
    </IonItem>
...
const Exporter = () => {
  ...
  const onExportClick = async () => {
    ...
    const appMode = await storage.get.appMode().catch(() => "regular");
    await exportConfig({
      cycles,
      language,
      theme,
