const bn = {
  // Home Tab
  Home: "হোম",
  "Period in": "পিরিয়ড হবে",
  "no info": "কোনো তথ্য নেই",
  Period: "পিরিয়ড",
  today: "আজ",
  Delay: "বিলম্ব",
  Days: "(0)[দিন];(1)[দিন];(2-4)[দিন];(5-20)[দিন];(21)[দিন];(22-24)[দিন];(25-30)[দিন];(31)[দিন];(32-34)[দিন];(35-40)[দিন];",
  "Current cycle day": "বর্তমান চক্রের দিন",
  day: "{{count}}তম দিন",
  Ovulation: "ডিম্বস্ফোটন",
  possible: "সম্ভবত আজ",
  finished: "শেষ",
  tomorrow: "আগামীকাল",
  in: "মধ্যে",
  "chance of getting pregnant": "গর্ভবতী হওয়ার সম্ভাবনা",
  High: "উচ্চ",
  Low: "নিম্ন",
  "Period today": "আজ পিরিয়ড",
  edit: "সম্পাদনা করুন",
  save: "সংরক্ষণ করুন",
  "Period is": "পিরিয়ড চলছে",
  "possible today": "আজ সম্ভব",

  // Details Tab
  Details: "বিস্তারিত",
  "Period length": "পিরিয়ডের দৈর্ঘ্য",
  "Cycle length": "চক্রের দৈর্ঘ্য",
  "You haven't marked any periods yet": "আপনি এখনও কোনো পিরিয়ড চিহ্নিত করেননি",

  // Mark Modal
  mark: "চিহ্নিত করুন",
  cancel: "বাতিল করুন",

  // Welcome Modal
  "Welcome to OVIFLOW": "OVIFLOW-তে স্বাগতম",
  "Mark the days of your": "আপনার দিনগুলি চিহ্নিত করুন",
  "last period": "শেষ পিরিয়ড",
  Continue: "চালিয়ে যান",
  "Forecast will not be generated.": "পূর্বাভাস তৈরি করা যাবে না।",
  or: "অথবা",

  // Info Modal
  "Frequent symptoms": "সাধারণ লক্ষণসমূহ",
  "is current phase of cycle": "- চক্রের বর্তমান পর্যায়",

  // Phases info
  "Menstrual phase": "মাসিক পর্যায়",
  "This cycle is accompanied by low hormone levels.":
    "এই চক্রে হরমোনের মাত্রা কম থাকে।",
  "lack of energy and strength": "শক্তি এবং বলের অভাব",
  pain: "ব্যথা",
  "weakness and irritability": "দুর্বলতা এবং বিরক্তি",
  "increased appetite": "ক্ষুধা বৃদ্ধি",

  "Follicular phase": "ফলিকুলার পর্যায়",
  "The level of estrogen in this phase rises and reaches a maximum level.":
    "এই পর্যায়ে ইস্ট্রোজেনের মাত্রা বৃদ্ধি পায় এবং সর্বোচ্চ স্তরে পৌঁছায়।",
  "strength and vigor appear": "শক্তি এবং উদ্যম দেখা যায়",
  "endurance increases": "সহনশীলতা বৃদ্ধি পায়",
  "new ideas and plans appear": "নতুন ধারণা এবং পরিকল্পনা দেখা যায়",
  "libido increases": "যৌন ইচ্ছা বৃদ্ধি পায়",

  "Ovulation phase": "ডিম্বস্ফোটন পর্যায়",
  "Once estrogen levels peak, they trigger the release of two important ovulation hormones, follicle-stimulating hormone and luteinizing hormone.":
    "একবার ইস্ট্রোজেনের মাত্রা শীর্ষে পৌঁছালে, এটি দুটি গুরুত্বপূর্ণ ডিম্বস্ফোটন হরমোনের মুক্তিকে ট্রিগার করে, ফলিকল-স্টিমুলেটিং হরমোন এবং লুটিনাইজিং হরমোন।",
  "increased sexual desire": "যৌন ইচ্ছা বৃদ্ধি",
  "optimistic mood": "আশাবাদী মেজাজ",
  "mild fever": "হালকা জ্বর",
  "lower abdominal pain": "নিম্ন উদরে ব্যথা",
  "chest discomfort and bloating": "বুকের অস্বস্তি এবং ফুলে যাওয়া",
  "characteristic secretions": "বৈশিষ্ট্যপূর্ণ স্রাব",

  "Luteal phase": "লুটিয়াল পর্যায়",
  "Levels of the hormones estrogen and progesterone first rise and then drop sharply just before a period. Progesterone reaches its peak in the luteal phase.":
    "ইস্ট্রোজেন এবং প্রোজেস্টেরন হরমোনের মাত্রা প্রথমে বৃদ্ধি পায় তারপর পিরিয়ডের ঠিক আগে তীব্রভাবে কমে যায়। লুটিয়াল পর্যায়ে প্রোজেস্টেরন তার শীর্ষে পৌঁছায়।",
  "breast tenderness": "স্তনের কোমলতা",
  puffiness: "ফুলে যাওয়া",
  "acne and skin rashes": "ব্রণ এবং ত্বকের ফুসকুড়ি",
  "diarrhea or constipation": "ডায়রিয়া বা কোষ্ঠকাঠিন্য",
  "irritability and depressed mood": "বিরক্তি এবং বিষণ্ণ মেজাজ",

  // Menu
  Preferences: "পছন্দসমূহ",
  Edit: "সম্পাদনা",
  Language: "ভাষা",
  Theme: "থিম",
  "Import config": "কনফিগ আমদানি করুন",
  "Export config": "কনফিগ রপ্তানি করুন",
  "Configuration has been imported": "কনফিগ সফলভাবে আমদানি করা হয়েছে",
  "Download latest version": "সর্বশেষ সংস্করণ ডাউনলোড করুন",
  "Stored cycles count": "সংরক্ষিত চক্রের সংখ্যা",

  // Alert Demo
  "This is just a demo": "এটি শুধুমাত্র একটি ডেমো",
  "You can download the application ":
    "আপনি অ্যাপ্লিকেশনটি ডাউনলোড করতে পারেন ",
  here: "এখানে",
  "We are on GitHub": "আমরা GitHub এ আছি",

  // Notifications
  Notifications: "বিজ্ঞপ্তিসমূহ",
  "Period is coming soon": "পিরিয়ড শীঘ্রই আসছে",
  "Your period may start tomorrow": "আপনার পিরিয়ড আগামীকাল শুরু হতে পারে",
  "Your period may start today": "আপনার পিরিয়ড আজ শুরু হতে পারে",

  // Count stored cycles
  "Confirm selection": "নির্বাচন নিশ্চিত করুন",
  "Are you sure you want to change the number of stored cycles?":
    "আপনি কি নিশ্চিত যে আপনি সংরক্ষিত চক্রের সংখ্যা পরিবর্তন করতে চান?",
  "Reducing the number will permanently remove some cycles.":
    "সংখ্যা কমানো হলে কিছু চক্র স্থায়ীভাবে মুছে যাবে।",

  // Settings
  Settings: "সেটিংস",
  About: "সম্পর্কে",
  Version: "সংস্করণ",
  "Profile & Account": "প্রোফাইল ও অ্যাকাউন্ট",
  Appearance: "চেহারা",
  "Dark Mode": "ডার্ক মোড",
  "Falling Flowers": "ঝরা ফুল",
  "Accent Color": "অ্যাকসেন্ট রঙ",
  "Cycle Preferences": "চক্র পছন্দসমূহ",
  "Health & Logging": "স্বাস্থ্য ও লগিং",
  "Doctor Details": "ডাক্তারের বিবরণ",
  "Privacy & Security": "গোপনীয়তা ও নিরাপত্তা",
  Data: "ডেটা",
  "App Lock": "অ্যাপ লক",
  "AI Insights": "AI অন্তর্দৃষ্টি",
  "Temperature Unit": "তাপমাত্রার একক",
  "Daily Water Goal": "দৈনিক পানির লক্ষ্য",
  "Default Cycle Length": "ডিফল্ট চক্রের দৈর্ঘ্য",
  "Default Period Length": "ডিফল্ট পিরিয়ডের দৈর্ঘ্য",
  "Luteal Phase Length": "লুটিয়াল পর্যায়ের দৈর্ঘ্য",
  "Ovulation Reminder": "ডিম্বস্ফোটন রিমাইন্ডার",
  "App Mode": "অ্যাপ মোড",
  Regular: "নিয়মিত",
  Pregnancy: "গর্ভাবস্থা",
  Postpartum: "প্রসবোত্তর",
  "Export Data (CSV)": "ডেটা রপ্তানি করুন (CSV)",
  "Download Health Report (PDF)": "স্বাস্থ্য রিপোর্ট ডাউনলোড করুন (PDF)",
  "Reset All Data": "সমস্ত ডেটা রিসেট করুন",
  "Delete Account": "অ্যাকাউন্ট মুছুন",
  Logout: "লগআউট",
  "Contact Doctor": "ডাক্তারের সাথে যোগাযোগ করুন",
  "Ask OVI": "OVI-কে জিজ্ঞাসা করুন",
  "When to See a Doctor": "কখন ডাক্তার দেখাবেন",
  "Today's Overview": "আজকের সারসংক্ষেপ",
  Welcome: "স্বাগতম",
  "Log Today's Cycle": "আজকের চক্র লগ করুন",
  "On period today?": "আজ পিরিয়ড?",
  Mood: "মেজাজ",
  Symptoms: "লক্ষণ",
  Notes: "নোট",
  "Cycle Health Score": "চক্র স্বাস্থ্য স্কোর",
  "Stay Hydrated": "জল পান করুন",
  "View History": "ইতিহাস দেখুন",
  "Saving…": "সংরক্ষণ হচ্ছে…",
  "Update Today's Log": "আজকের লগ আপডেট করুন",
  "Save Today's Log": "আজকের লগ সংরক্ষণ করুন",
  "Pregnancy Log": "গর্ভাবস্থা লগ",
  "Postpartum Log": "প্রসবোত্তর লগ",
  "Next Period": "পরবর্তী পিরিয়ড",
  "days left": "দিন বাকি",
  Cycle: "চক্র",
  Pregnant: "গর্ভবতী",
};

export default bn;
