import React from "react";
import { format, parseISO } from "date-fns";

interface PregnancyLog {
  date: string;
  waterGlasses: number;
  vitaminsTaken: boolean;
  symptoms: string[];
  notes?: string;
}

interface PregnancyHistoryProps {
  logs: PregnancyLog[];
}

const PregnancyHistory: React.FC<PregnancyHistoryProps> = ({ logs }) => {
  if (logs.length === 0) {
    return <p className="history-empty">No pregnancy logs recorded yet.</p>;
  }

  return (
    <div className="history-container">
      {logs.map((log) => (
        <div key={log.date} className="history-item">
          <div className="history-header">
            <span className="history-date">
              {format(parseISO(log.date), "MMM d, yyyy")}
            </span>
            {log.vitaminsTaken && (
              <span className="history-badge vitamin">Vitamin ✓</span>
            )}
          </div>
          <div className="history-details">
            <p>💧 {log.waterGlasses} glasses of water</p>
            {log.symptoms.length > 0 && (
              <p>⚠️ Symptoms: {log.symptoms.join(", ")}</p>
            )}
            {log.notes && <p className="history-notes">"{log.notes}"</p>}
          </div>
        </div>
      ))}
    </div>
  );
};

export default PregnancyHistory;
