...
export const storage = {
  set: {
    ...
    appMode: (value: StorageValueType<StorageKey.AppMode>) => {
      return Preferences.set({
        key: StorageKey.AppMode,
        value: value,
      });
    },
  },
  get: {
    ...
    appMode: async () => {
      const { value } = await Preferences.get({ key: StorageKey.AppMode });
      if (!value) {
        throw new Error(`Can't find '${StorageKey.AppMode}' in storage`);
      }
      return value;
    },
  },
